...
HELP_TEXTS = {
    ...
    按日期将所有构件分配到多天，支持“后面的日子优先”策略。

    流程

    选择模式：1。

    如存在“支撑”：在进入“支撑”配置前选择分桶策略（编号 / 楼层）。
    ...
    预览分配；可选择“未分配条目合并到最后一天”。

    写入页池；按天批量写元信息（日期、温度、仪器）。

    输出

    工作表命名沿用模板；K33=日期，K34=温度。
    ...
    定义楼层桶：支持 B3-B1、1F-3F、屋面、机房层 等；顺序自动规范。

    为每个桶指定日期与（可选）温度，或使用统一设置。
    ...
    日期：20250101、2025年1月1日、2025 1 1""",
        "4": """Mode 4｜楼层 × 日期 切片

    适用场景

    同一楼层需分配到多天；可选择均分或每日上限（配额）切片。

    概念

    共用计划：同一批选定楼层共用一套“日期 + 每日上限（可空）”。

    默认计划（*）：对未单独配置的楼层套用的通用计划。

    兜底：仍未分配的条目可：

    A）统一日期/温度一次性分配；或

    B）回落至 Mode 1（日期分桶）流程。
    ...
    y：一次录入日期清单与每日上限（留空=均分）→ 应用于所选楼层。

    回车：逐楼层分别录入日期与每日上限。

    如仍存在未配置楼层：是否创建**默认计划（*）**用于剩余楼层？

    y：录入一次日期与每日上限，作为默认计划；
    ...
    询问是否使用统一日期/温度一次性分配；若否 → 回落至 Mode 1（日期分桶）流程。
